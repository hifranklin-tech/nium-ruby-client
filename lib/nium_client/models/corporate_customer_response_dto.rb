...
module NiumClient
  class CorporateCustomerResponseDTO
    ...
    def self.openapi_types
      {
        :'business_details' => :'BusinessDetailsResponseV2DTO',
        :'case_id' => :'String',
        :'client_id' => :'String',
        :'compliance_region' => :'String',
        :'risk_assessment_info' => :'RiskAssessmentInfoV2'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::CorporateCustomerResponseDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::CorporateCustomerResponseDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [business_details, case_id, client_id, compliance_region, risk_assessment_info].hash
    end
