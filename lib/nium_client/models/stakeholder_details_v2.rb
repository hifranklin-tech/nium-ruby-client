  ...
  class StakeholderDetailsV2
    ...
    def self.openapi_types
      {
        ...
        :'contact_details' => :'StakeholderContactDetailsResponseDTO',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::StakeholderDetailsV2` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::StakeholderDetailsV2`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [address, birth_country, contact_details, date_of_birth, document_details, first_name, gender, is_pep, last_name, middle_name, nationality, professional_details, tax_details].hash
    end
