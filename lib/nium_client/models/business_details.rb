  ...
  class BusinessDetails
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::BusinessDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::BusinessDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [additional_info, addresses, applicant_details, association_details, bank_account_details, business_name, business_registration_number, business_type, description, document_details, former_name, legal_details, partnership_details, reference_id, regulatory_details, settlor_name, stakeholders, stock_symbol, tax_details, trade_name, trustee_name, website].hash
    end
