  ...
  class AddCardRequestVIRUP2PHY
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::AddCardRequestVIRUP2PHY` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::AddCardRequestVIRUP2PHY`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [card_expiry, card_product_id, card_type, name_on_card, additional_line, email, child_customer_hash_id, country_code, mobile, issuance_mode, plastic_id, delivery].hash
    end
