...
module NiumClient
  class Consent
    ...
    attr_accessor :frequency_per_day
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::Consent` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::Consent`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'frequency_per_day')
        self.frequency_per_day = attributes[:'frequency_per_day']
      end
      ...
      if attributes.key?(:'sca_status')
        self.sca_status = attributes[:'sca_status']
      end
    ...
    def hash
      [access, consent_status, frequency_per_day, last_action_date, recurring_indicator, sca_status, valid_until].hash
    end
