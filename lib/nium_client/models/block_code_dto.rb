  ...
  class BlockCodeDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::BlockCodeDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::BlockCodeDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'block_action')
        self.block_action = attributes[:'block_action']
      else
        self.block_action = nil
      end
    ...
    def list_invalid_properties
      ...
      if @block_action.nil?
        invalid_properties.push('invalid value for "block_action", block_action cannot be nil.')
      end
    ...
    def valid?
      ...
      reason_validator = EnumAttributeValidator.new('String', ["fraud, cardLost, cardStolen,damaged"])
      return false unless reason_validator.valid?(@reason)
      return false if @block_action.nil?
    ...
    def hash
      [reason, block_action, remarks, comments].hash
    end
