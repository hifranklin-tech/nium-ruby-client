...
module NiumClient
  class CustomerLinkAccountRequest
    ...
    def self.openapi_types
      {
        ...
        :'routing_codes' => :'Array<RoutingInfo>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::CustomerLinkAccountRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::CustomerLinkAccountRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'routing_codes')
        if (value = attributes[:'routing_codes']).is_a?(Array)
          self.routing_codes = value
        end
      end
    end
    ...
    def hash
      [account_number, country, currency, funding_channel, routing_codes].hash
    end
