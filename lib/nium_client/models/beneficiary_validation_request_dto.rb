...
module NiumClient
  class BeneficiaryValidationRequestDTO
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NiumClient::BeneficiaryValidationRequestDTO` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NiumClient::BeneficiaryValidationRequestDTO`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [beneficiary_account_number, beneficiary_account_type, beneficiary_bank_code, beneficiary_contact_number, beneficiary_country_code, beneficiary_name, destination_country, destination_currency, payout_method, proxy_type, proxy_value, routing_code_type1, routing_code_value1].hash
    end
